...
model <- cmdstan_model(
  "depot_2cmt_linear/Stan/Torsten/Predict/depot_2cmt_prop_predict_new_subjects.stan"
)
...
# data <- read_csv("depot_2cmt_linear/data/single_dose.csv",
data <- read_csv("depot_2cmt_linear/data/multiple_dose.csv",
  na = "."
) %>%
  rename_all(tolower) %>%
  rename(
