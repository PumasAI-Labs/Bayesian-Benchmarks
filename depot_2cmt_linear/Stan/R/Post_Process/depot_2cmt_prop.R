...
# nonmem_data <- read_csv("depot_2cmt_linear/data/single_dose.csv",
nonmem_data <- read_csv("depot_2cmt_linear/data/multiple_dose.csv",
  na = "."
) %>%
  rename_all(tolower) %>%
  rename(
  ...
  ) # This value can be anything except NA. It'll be indexed away
...
## Read in fit
# fit <- read_rds("depot_2cmt_linear/Stan/Torsten/Fits/single_dose.rds")
fit <- read_rds("depot_2cmt_linear/Stan/Torsten/Fits/multiple_dose.rds")


## Summary of parameter estimates
parameters_to_summarize <- c(
  str_subset(fit$metadata()$stan_variables, "TV"),
  str_c("omega_", c("cl", "vc", "q", "vp", "ka")),
  "sigma_p"
)

summary <- summarize_draws(
  fit$draws(parameters_to_summarize),
...
summary %>%
  ...
  select(
    variable, mean, sd, q2.5, median, q97.5, rse, rhat,
    starts_with("ess")
  ) %>%
  print(n = 50)
...
post_preds_summary <- draws_df %>%
  ...
  mutate(
    DV = nonmem_data$DV[nonmem_data$evid == 0][i],
...
animate(some_residuals_qq,
  nframes = 100, width = 384, height = 384, res = 96,
  dev = "png", type = "cairo"
)
...
animate(some_residuals_scatter,
  nframes = 30, width = 384, height = 384,
