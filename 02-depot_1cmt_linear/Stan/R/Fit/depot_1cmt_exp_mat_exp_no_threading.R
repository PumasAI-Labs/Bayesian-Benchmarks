...
read_data_and_prepare_stan_data <- function(n_doses = c("single", "multiple")){
  ...
  nonmem_data <- read_csv(str_c("02-depot_1cmt_linear/data/", 
                                n_doses, "_dose.csv"),
                          na = ".") %>% 
    rename_all(tolower) %>% 
    rename(ID = "id",
...
sample_and_save_all <- function(n_doses = c("single", "multiple"), run_number){
  ...
  init_files <- str_c("02-depot_1cmt_linear/data/inits/inits_", run_number, "_", 
                      1:4, ".json")
  
  stan_data <- read_data_and_prepare_stan_data(n_doses = n_doses)
  
  model <- cmdstan_model(
    "02-depot_1cmt_linear/Stan/Torsten/Fit/depot_1cmt_exp_mat_exp_no_threading.stan")
  ...
  fit$save_object(str_c("02-depot_1cmt_linear/Stan/Torsten/Fits/", 
                        n_doses, "_dose_mat_exp_no_threading_", 
                        run_number, ".rds"))
  ...
  fit$draws(parameters_to_summarize, format = "draws_df") %>% 
    as_tibble() %>% 
    write_csv(str_c("02-depot_1cmt_linear/Stan/Torsten/Fits/", 
                    n_doses, "_dose_draws_df_mat_exp_no_threading_", 
                    run_number, ".csv"))
