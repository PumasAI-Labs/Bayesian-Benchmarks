...
function devech(x::StridedVector; islower=true)
    n = div(isqrt(8*length(x) + 1) - 1, 2)
    if n*(n + 1) != 2*length(x)
        throw(DimensionMismatch("length of input vector ($length(x)) is incompatible with the operation"))
    end
...
init_params = (
    ...
    0.0168748417643117
    0.0168748417643117
    0.0723822402125386]; islower=false))
