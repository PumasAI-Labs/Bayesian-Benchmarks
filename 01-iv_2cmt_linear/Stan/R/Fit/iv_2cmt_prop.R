...
read_data_and_prepare_stan_data <- function(n_doses = c("single", "multiple")){
  ...
  nonmem_data <- read_csv(str_c("01-iv_2cmt_linear/data/", n_doses, "_dose.csv"),
                          na = ".") %>% 
    rename_all(tolower) %>% 
    rename(ID = "id",
           DV = "dv") %>% 
...
sample_and_save_all <- function(n_doses = c("single", "multiple"), run_number){
  ...
  init_files <- str_c("01-iv_2cmt_linear/data/inits/inits_", run_number, "_", 
                      1:4, ".json")
  
  stan_data <- read_data_and_prepare_stan_data(n_doses = n_doses)
  
  model <- cmdstan_model(
    "01-iv_2cmt_linear/Stan/Torsten/Fit/iv_2cmt_prop.stan",
  ...
  fit$save_object(str_c("01-iv_2cmt_linear/Stan/Torsten/Fits/", 
  ...
  fit$draws(parameters_to_summarize, format = "draws_df") %>% 
    as_tibble() %>% 
    write_csv(str_c("01-iv_2cmt_linear/Stan/Torsten/Fits/", 
