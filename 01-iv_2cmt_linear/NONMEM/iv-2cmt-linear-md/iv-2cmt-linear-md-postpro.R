...
modelName <- "iv-2cmt-linear-md"
thetas <- c("CL","V1", "Q", "V2")
randoms <- c("OMEGA_CL", "OMEGA_V1", "OMEGA_Q", "OMEGA_V2", "SIGMA")
...
for(i in 1:nChains) {
  params <- read.delim(paste0(modelDir, "/", modelName,"-", i, ".ext"), sep="", skip=1) %>% 
    select(ITERATION, CL=THETA1, V1=THETA2, Q=THETA3, V2=THETA4, 
           OMEGA_CL=OMEGA.1.1., OMEGA_V1=OMEGA.2.2., OMEGA_Q=OMEGA.3.3., OMEGA_V2=OMEGA.4.4., 
           SIGMA=SIGMA.1.1.) %>% 
    filter(ITERATION>0) %>% 
    mutate(Chain=i)
