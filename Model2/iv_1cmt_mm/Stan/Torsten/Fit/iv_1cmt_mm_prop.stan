...
functions{
  ...
  real partial_sum_lpmf(array[] int seq_subj, int start, int end,
    ...
    for(n in 1:N){            // loop over subjects in this slice
      ...
      // real km = theta_nn[3];
      // 
      // array[n_random + 2] real theta_params = {0, vc, vmax, km, 0}; 
...
generated quantities{
 ...
 
  {
    ...
    // 
    // array[n_random + 1] real theta_params_tv = {0, TVVC, TVVMAX, TVKM, 0};
    // 
    // // eta_cl = col(eta, 1);
    // eta_vc = col(eta, 1);
    // eta_vmax = col(eta, 2);
    ...
    // 
    //   array[n_random + 1] real theta_params = {0, VC[j], VMAX[j], KM[j], 0};
    //   
