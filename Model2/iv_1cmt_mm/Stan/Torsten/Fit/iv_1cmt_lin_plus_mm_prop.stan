...
functions{
  ...
  real partial_sum_lpmf(array[] int seq_subj, int start, int end,
                        ...
                        real TVCL, real TVVC, real TVVMAX, real TVKM,
    ...
    row_vector[n_random] typical_values = 
                                      to_row_vector({TVCL, TVVC, TVVMAX, TVKM});
    ...
    for(n in 1:N){            // loop over subjects in this slice
      ...
      // real cl = theta_nn[1];
      // real vc = theta_nn[2];
      // real vmax = theta_nn[3];
      // real km = theta_nn[4];
      // 
      // array[n_random + 1] real theta_params = {cl, v, vmax, km, 0}; 
...
model{ 
  ...
  target += reduce_sum(partial_sum_lupmf, seq_subj, grainsize,
                       ...
                       TVCL, TVVC, TVVMAX, TVKM, omega, L, Z,
                       sigma_p,
                       lloq, bloq,
...
generated quantities{
 ...
 
  {
    ...
    // 
    // array[n_random + 1] real theta_params_tv = {TVCL, TVVC, TVVMAX, TVKM, 0};
    // 
    // eta_cl = col(eta, 1);
    // eta_vc = col(eta, 2);
    // eta_vmax = col(eta, 3);
    ...
    // omega_vc_vmax = Omega[2, 3];
    // omega_vc_km = Omega[2, 4];
    // omega_vmax_km = Omega[3, 4];

    // for(j in 1:n_subjects){
    // 
    //   array[n_random + 1] real theta_params = {CL[j], VC[j], VMAX[j], KM[j], 0};
    //   
