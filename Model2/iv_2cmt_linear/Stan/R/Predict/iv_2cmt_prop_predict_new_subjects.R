...
model <- cmdstan_model(
  "Model2/iv_2cmt_linear/Stan/Torsten/Predict/iv_2cmt_prop_predict_new_subjects.stan")
...
data <- read_csv("Model2/iv_2cmt_linear/Data/single_dose.csv",
# data <- read_csv("Model2/iv_2cmt_linear/Data/multiple_dose.csv",
                 na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
