...
fit <- read_rds("Model2/iv_2cmt_linear/Stan/Torsten/Fits/single_dose.rds")
# fit <- read_rds("Model2/iv_2cmt_linear/Stan/Torsten/Fits/multiple_dose.rds")

nonmem_data <- read_csv("Model2/iv_2cmt_linear/Data/single_dose.csv",
# nonmem_data <- read_csv("Model2/iv_2cmt_linear/Data/multiple_dose.csv",
                        na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
...
model <- cmdstan_model(
  "Model2/iv_2cmt_linear/Stan/Torsten/Predict/iv_2cmt_prop_predict_observed_subjects.stan")
...
ggplot(preds_ind, aes(x = time, group = ID)) +
