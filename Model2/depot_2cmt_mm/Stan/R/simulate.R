...
model_simulate <- 
  cmdstan_model(
    "Model2/depot_2cmt_mm/Stan/Torsten/Simulate/depot_2cmt_lin_plus_mm_ppa.stan") 
...
dosing_data <- expand.ev(ID = 1:n_subjects_per_dose, addl = 0, ii = 0, 
                         cmt = 1, amt = c(10, 20, 40, 80), tinf = 0,
                         evid = 1) %>%
  as_tibble() %>% 
  rename_all(toupper) %>%
  mutate(SS = 0) %>% 
...
params_ind <- simulated_data$draws(c("CL", "VC", "Q", "VP", "VMAX", "KM", 
                                     "KA")) %>% 
  spread_draws(CL[i], VC[i], Q[i], VP[i], VMAX[i], KM[i], KA[i]) %>% 
  inner_join(dosing_data %>% 
               mutate(i = 1:n()),
             by = "i") %>% 
  ungroup() %>%
  select(ID, CL, VC, Q, VP, VMAX, KM, KA)
...
data %>% 
  write_csv(file.path("Model2", "depot_2cmt_mm", "Data", "single_dose.csv"),
            na = ".")

params_ind %>% 
  write_csv("Model2/depot_2cmt_mm/Data/single_dose_params_ind.csv")

################ Now Simulate Multiple Doses ####################

dosing_data <- expand.ev(ID = 1:n_subjects_per_dose, addl = 6, ii = 24, 
                         cmt = 2, amt = c(10, 20, 40, 80), tinf = 1,
                         evid = 1) %>%
  as_tibble() %>% 
  rename_all(toupper) %>%
  mutate(SS = 0) %>% 
...
data %>% 
  write_csv(file.path("Model2", "depot_2cmt_mm", "Data", "multiple_dose.csv"),
            na = ".")

params_ind %>% 
  write_csv("Model2/depot_2cmt_mm/Data/multiple_dose_params_ind.csv")
