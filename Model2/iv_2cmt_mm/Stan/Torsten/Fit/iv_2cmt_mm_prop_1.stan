...
functions{
  ...
  vector iv_2cmt_mm_ode(real t, vector y, array[] real params, 
                        array[] real x_r, array[] int x_i){
    ...
    dydt[1] = -vmax*conc/(km + conc) + k_pc*y[2];
    dydt[2] = k_cp*y[1] - k_pc*y[2];
  ...
  real partial_sum_lpmf(array[] int seq_subj, int start, int end,
    ...
    for(n in 1:N){            // loop over subjects in this slice
      ...
      // real km = theta_nn[5];
      // 
      // array[n_random] real theta_params = {vc, q, vp, vmax, km}; 
...
model{ 
  ...
  target += reduce_sum(partial_sum_lupmf, seq_subj, grainsize,
                       ...
                       TVVC, TVQ, TVVP, TVVMAX, TVKM, omega, L, Z,
                       sigma_p,
                       lloq, bloq,
...
generated quantities{
 ...
 
  {
    ...
    // matrix[n_total, n_cmt] x_ipred;
    // 
    // array[n_random] real theta_params_tv = {TVVC, TVQ, TVVP, TVVMAX, TVKM};
    //                                             
    // 
    // // eta_cl = col(eta, 1);
    // eta_vc = col(eta, 1);
    ...
    // 
    //   array[n_random] real theta_params = {VC[j], Q[j], VP[j], VMAX[j], 
    //                                        KM[j]};
