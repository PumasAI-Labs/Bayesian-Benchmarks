...
model_simulate <- 
  cmdstan_model(
    "Model2/iv_2cmt_mm/Stan/Torsten/Simulate/iv_2cmt_lin_plus_mm_ppa.stan") 
...
params_ind <- simulated_data$draws(c("CL", "VC", "Q", "VP", "VMAX", "KM")) %>% 
  spread_draws(CL[i], VC[i], Q[i], VP[i], VMAX[i], KM[i]) %>% 
  inner_join(dosing_data %>% 
               mutate(i = 1:n()),
             by = "i") %>% 
  ungroup() %>%
  select(ID, CL, VC, Q, VP, VMAX, KM)
...
data %>% 
  write_csv(file.path("Model2", "iv_2cmt_mm", "Data", "single_dose.csv"),
            na = ".")

params_ind %>% 
  write_csv("Model2/iv_2cmt_mm/Data/single_dose_params_ind.csv")
...
data %>% 
  write_csv(file.path("Model2", "iv_2cmt_mm", "Data", "multiple_dose.csv"),
            na = ".")

params_ind %>% 
  write_csv("Model2/iv_2cmt_mm/Data/multiple_dose_params_ind.csv")
