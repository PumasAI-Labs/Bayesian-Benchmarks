...
# fit <- read_rds("Model2/iv_2cmt_mm/Stan/Torsten/Fits/multiple_dose.rds")
...
# dosing_data <- mrgsolve::expand.ev(addl = 6, ii = 24, cmt = 2,
                                   amt = c(10, 20, 40, 80, 160, 320), tinf = 1,
                                   evid = 1, mdv = 1) %>%
  as_tibble() %>% 
  mutate(ss = 0) %>% 
...
model <- cmdstan_model(
  "Model2/iv_2cmt_mm/Stan/Torsten/Predict/iv_2cmt_mm_prop_predict_new_subjects.stan")
...
data <- read_csv("Model2/iv_2cmt_mm/Data/single_dose.csv",
# data <- read_csv("Model2/iv_2cmt_mm/Data/multiple_dose.csv",
                 na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
...
preds_ind %>% 
  ...
  theme(axis.text = element_text(size = 14, face = "bold"),
        axis.title = element_text(size = 18, face = "bold"),
        legend.position = "bottom") +
  facet_wrap(~ regimen, nrow = 1, ncol = 4)
