...
nonmem_data <- read_csv("Model2/iv_2cmt_mm/Data/single_dose.csv",
# nonmem_data <- read_csv("Model2/iv_2cmt_mm/Data/multiple_dose.csv",
                        na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
...
## Read in fit
# fit <- read_rds("Model2/iv_2cmt_mm/Stan/Torsten/Fits/single_dose_1.rds")
fit <- read_rds("Model2/iv_2cmt_mm/Stan/Torsten/Fits/multiple_dose_1.rds")


## Summary of parameter estimates
parameters_to_summarize <- c(str_subset(fit$metadata()$stan_variables, "TV"),
                             str_c("omega_", c("vc", "q", "vp", "vmax", "km")),
                             "sigma_p")
...
mcmc_combo(fit$draws(c("TVVC", "TVQ", "TVVP", "TVVMAX", "TVKM")),
           combo = c("dens_overlay", "trace"))
mcmc_combo(fit$draws(c("omega_vc", "omega_q", "omega_vp", "omega_vmax", 
                       "omega_km")),
           combo = c("dens_overlay", "trace"))
...
est_stan <- draws_df %>%
  spread_draws(VC[ID], Q[ID], VP[ID], VMAX[ID], KM[ID],
               eta_vc[ID], eta_q[ID], eta_vp[ID], eta_vmax[ID], eta_km[ID]) %>% 
  mean_qi() %>% 
  select(ID, VC, Q, VP, VMAX, KM, 
         eta_vc, eta_q, eta_vp, eta_vmax, eta_km)
