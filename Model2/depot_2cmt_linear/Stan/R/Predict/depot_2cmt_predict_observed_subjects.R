...
# nonmem_data <- read_csv("Model2/depot_2cmt_linear/Data/single_dose.csv",
nonmem_data <- read_csv("Model2/depot_2cmt_linear/Data/multiple_dose.csv",
                        na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
...
model <- cmdstan_model(
  "Model2/depot_2cmt_linear/Stan/Torsten/Predict/depot_2cmt_prop_predict_observed_subjects.stan")
