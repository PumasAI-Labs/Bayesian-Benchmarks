...
model <- cmdstan_model(
  "Model2/depot_1cmt_mm/Stan/Torsten/Predict/depot_1cmt_mm_prop_predict_new_subjects.stan")
...
# data <- read_csv("Model2/depot_1cmt_mm/Data/single_dose.csv",
data <- read_csv("Model2/depot_1cmt_mm/Data/multiple_dose.csv",
                 na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
