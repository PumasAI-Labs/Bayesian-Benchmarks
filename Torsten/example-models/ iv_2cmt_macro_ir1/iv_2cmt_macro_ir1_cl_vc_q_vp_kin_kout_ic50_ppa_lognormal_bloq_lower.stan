...
// Macro parameterization (CL, VC, Q, VP) for PK
// KIN, KOUT, IC50, IMAX for PD
// IIV on CL, VC, Q, VP
// IIV on KIN, KOUT, IC50. IMAX is fixed to be 1
...
functions{
  ...
  vector iv_2cmt_macro_ir1_ode(real t, vector y, array[] real params, 
  ...
  real partial_sum_lpmf(array[] int seq_subj, int start, int end,
    ...
    matrix[n_total, n_cmt] x_ipred; // = rep_matrix(0, n_total, 2);
    ...
    for(n in 1:N){            // loop over subjects in this slice
      ...
      x_ipred[subj_start[nn]:subj_end[nn], ] =
        pmx_solve_rk45(iv_2cmt_macro_ir1_ode,
                       n_cmt,
