...
times_new <- tibble(time = sort(unique(c(t1, seq(0, 24, by = 0.5)))))
...
model <- cmdstan_model(
  "Model1/Stan/Torsten/Predict/depot_1cmt_prop_predict_new_subjects.stan")
...
data <- read_csv("Model1/Data/001.csv",
         na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
