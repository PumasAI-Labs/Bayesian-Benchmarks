...
nonmem_data <- read_csv("Model1/Data/005.csv",
                        na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
...
time_chains <- fit$time()$chains
time <- mean(time_chains$total)
convert_csv_to_arrow(fit$output_files(), file.path("Model1/Stan/Torsten/Fits/"), time)
