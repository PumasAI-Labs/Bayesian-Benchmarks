...
fit <- read_rds("04-depot_1cmt_mm/Stan/Torsten/Fits/single_dose_1.rds")
# fit <- read_rds("04-depot_1cmt_mm/Stan/Torsten/Fits/multiple_dose_1.rds")
...
dosing_data <- mrgsolve::expand.ev(addl = 0, ii = 0, cmt = 1,
                                   ...
                                   amt = c(10, 20, 40, 80, 160, 320), tinf = 0,
                                   evid = 1, mdv = 1) %>%
  as_tibble() %>% 
  mutate(ss = 0) %>% 
...
model <- cmdstan_model(
  "04-depot_1cmt_mm/Stan/Torsten/Predict/depot_1cmt_mm_exp_predict_new_subjects.stan")
...
tmp <- ggplot(preds_ind, aes(x = time, group = ID)) +
  geom_line(aes(y = cp), linetype = 1, linewidth = 1.15) +
...
data <- read_csv("04-depot_1cmt_mm/data/single_dose.csv",
# data <- read_csv("04-depot_1cmt_mm/data/multiple_dose.csv",
                 na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
