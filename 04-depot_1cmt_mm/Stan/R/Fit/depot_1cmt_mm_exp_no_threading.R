...
read_data_and_prepare_stan_data <- function(n_doses = c("single", "multiple")){
  ...
  nonmem_data <- read_csv(str_c("04-depot_1cmt_mm/data/", 
                                n_doses, "_dose.csv"),
                          na = ".") %>% 
    rename_all(tolower) %>% 
    rename(ID = "id",
...
sample_and_save_all <- function(n_doses = c("single", "multiple"), run_number){
  ...
  init_files <- str_c("04-depot_1cmt_mm/data/inits/inits_", run_number, "_", 
                      1:4, ".json")
  
  stan_data <- read_data_and_prepare_stan_data(n_doses = n_doses)
  
  model <- cmdstan_model(
    "04-depot_1cmt_mm/Stan/Torsten/Fit/depot_1cmt_mm_exp_no_threading.stan")
  ...
  fit$save_object(str_c("04-depot_1cmt_mm/Stan/Torsten/Fits/", 
                        n_doses, "_dose_no_threading_", run_number, ".rds"))
  ...
  fit$draws(parameters_to_summarize, format = "draws_df") %>% 
    as_tibble() %>% 
    write_csv(str_c("04-depot_1cmt_mm/Stan/Torsten/Fits/", 
                    n_doses, "_dose_draws_df_no_threading_", 
                    run_number, ".csv"))
