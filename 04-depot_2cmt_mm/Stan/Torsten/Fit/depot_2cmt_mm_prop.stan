...
functions{
  ...
  real partial_sum_lpmf(array[] int seq_subj, int start, int end,
                        ...
                        real TVVC, real TVQ, real TVVP, real TVVMAX, real TVKM,
                        real TVKA,
    ...
    for(n in 1:N){            // loop over subjects in this slice
      ...
      // real q = theta_nn[2];
      // real vp = theta_nn[3];
      // real vmax = theta_nn[4];
      // real km = theta_nn[5];
      // real ka = theta_nn[6];
      // 
      // array[n_random + 1] real theta_params = {0, vc, q, vp, vmax, km, ka}; 
      
      array[n_random + 1] real theta_params = 
                    to_array_1d(append_col(0, theta[nn])); // access the parameters for subject nn
      ...
      dv_ipred[subj_start[nn]:subj_end[nn]] = 
        x_ipred[subj_start[nn]:subj_end[nn], 2] ./ theta_params[2];
...
transformed data{ 
  ...
  int n_cmt = 3;                       // Number of compartments in ODEs
  
  array[n_random] real scale_omega = {scale_omega_vc, scale_omega_q, 
                                      scale_omega_vp, scale_omega_vmax, 
...
model{ 
  ...
  target += reduce_sum(partial_sum_lupmf, seq_subj, grainsize,
                       ...
                       TVVC, TVQ, TVVP, TVVMAX, TVKM, TVKA, omega, L, Z,
                       sigma_p,
                       lloq, bloq,
...
generated quantities{
  ...
  real<lower = 0> omega_q = omega[2];
  real<lower = 0> omega_vp = omega[3];
  real<lower = 0> omega_vmax = omega[4];
  real<lower = 0> omega_km = omega[5];
  real<lower = 0> omega_ka = omega[6];
 ...
 
  {
    ...
    // 
    // array[n_random + 1] real theta_params_tv = {0, TVVC, TVQ, TVVP, TVVMAX, 
    //                                             TVKM, TVKA};
    // 
    // // eta_cl = col(eta, 1);
    // eta_vc = col(eta, 1);
    ...
    // // real cor_cl_ka;
    // real cor_vc_q = R[1, 2];
    // real cor_vc_vp = R[1, 3];
    // real cor_vc_vmax = R[1, 4];
    // real cor_vc_km = R[1, 5];
    // real cor_vc_ka = R[1, 6];
    // real cor_q_vp = R[2, 3];
    // real cor_q_vmax = R[2, 4];
    // real cor_q_km = R[2, 5];
    // real cor_q_ka = R[2, 6];
    // real cor_vp_vmax = R[3, 4];
    // real cor_vp_km = R[3, 5];
    // real cor_vp_ka = R[3, 6];
    // real cor_vmax_km = R[4, 5];
    // real cor_vmax_ka = R[4, 6];
    // real cor_km_ka = R[5, 6];
    ...
    // // real omega_cl_km;
    // // real omega_cl_ka;
    // real omega_vc_q = Omega[1, 2];
    // real omega_vc_vp = Omega[1, 3];
    // real omega_vc_vmax = Omega[1, 4];
    // real omega_vc_km = Omega[1, 5];
    // real omega_vc_ka = Omega[1, 6];
    // real omega_q_vp = Omega[2, 3];
    // real omega_q_vmax = Omega[2, 4];
    // real omega_q_km = Omega[2, 5];
    // real omega_q_ka = Omega[2, 6];
    // real omega_vp_vmax = Omega[3, 4];
    // real omega_vp_km = Omega[3, 5];
    // real omega_vp_ka = Omega[3, 6];
    // real omega_vmax_km = Omega[4, 5];
    // real omega_vmax_ka = Omega[4, 6];
    // real omega_km_ka = Omega[5, 6];

    // for(j in 1:n_subjects){
    // 
    //   array[n_random + 1] real theta_params = {0, VC[j], Q[j], VP[j], 
    //                                            VMAX[j], KM[j], KA[j]};
