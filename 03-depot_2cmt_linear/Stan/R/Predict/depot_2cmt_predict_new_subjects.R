...
dosing_data <- mrgsolve::expand.ev(
  addl = 6, ii = 24, cmt = 1,
...
new_data <- bind_rows(replicate(max(dosing_data$ID), times_new,
  ...
  mutate(
    ID = rep(1:max(dosing_data$ID), each = nrow(times_new)),
    amt = 0,
    evid = 0,
    rate = 0,
    addl = 0,
...
model <- cmdstan_model(
  "03-depot_2cmt_linear/Stan/Torsten/Predict/depot_2cmt_prop_predict_new_subjects.stan"
)
...
# data <- read_csv("03-depot_2cmt_linear/data/single_dose.csv",
data <- read_csv("03-depot_2cmt_linear/data/multiple_dose.csv",
  na = "."
) %>%
  rename_all(tolower) %>%
  rename(
...
preds_ind %>%
  ...
  geom_point(
    data = data %>%
      filter(
        regimen %in% str_c(c(10, 30, 60, 120), " mg"),
        mdv == 0
      ),
