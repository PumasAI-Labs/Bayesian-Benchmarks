...
model_simulate <- cmdstan_model(
  "03-depot_2cmt_linear/Stan/Torsten/Simulate/depot_2cmt_ppa.stan"
)
...
dosing_data <- expand.ev(
  ID = 1:n_subjects_per_dose, addl = 0, ii = 0,
  cmt = 1, amt = c(120), tinf = 0,
  evid = 1
) %>%
  as_tibble() %>%
  rename_all(toupper) %>%
...
times_to_simulate <- seq(0.25, 72, by = 0.25)
...
simulated_data <- model_simulate$sample(
  data = stan_data,
  fixed_param = TRUE,
  seed = 54321,
  iter_warmup = 0,
...
ggplot(data %>%
  ...
  theme_bw(18) +
  scale_y_continuous(
    name = latex2exp::TeX("Drug Conc. $(\\mu g/mL)"),
    trans = "log10"
  ) +
  scale_x_continuous(
    name = "Time (h)",
